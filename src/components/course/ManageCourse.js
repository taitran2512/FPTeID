...
const Item = ({ courseName, trainer, created_by, startedDate, endedDate, buildingName, roomName }) => (
   <ItemCourse
      ...
      const renderItem = ({ item, index }) => (
         <TouchableOpacity
            ...
            <Item
               ...
               startedDate={
                  !stringIsEmpty(item.startedDate)
                     ? item.startedDate.slice(0, 10).split('-').reverse().join('/')
                     : item.startedDate
               }
               endedDate={
                  !stringIsEmpty(item.endedDate)
                     ? item.endedDate.slice(0, 10).split('-').reverse().join('/')
                     : item.endedDate
               }
